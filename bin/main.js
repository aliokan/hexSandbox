...
var Main = function() {
	window.console.debug("test");
	var applicationAssembler = new hex_runtime_ApplicationAssembler();
	var applicationContext = applicationAssembler.getApplicationContext("applicationContext",hex_ioc_assembler_ApplicationContext);
	...
	hex_domain_DomainExpert.getInstance().registerDomain(hex_domain_DomainUtil.getDomain("module2",hex_domain_Domain));
	hex_metadata_AnnotationProvider.registerToParentDomain(hex_domain_DomainUtil.getDomain("module2",hex_domain_Domain),hex_domain_DomainUtil.getDomain("applicationContext",hex_domain_Domain));
	var module2 = new mock_MockModule();
	coreFactory.register("module2",module2);
	var myService = new mock_MockService();
	__applicationContextInjector.mapClassNameToValue("mock.IMockService",myService,"myService");
	coreFactory.register("myService",myService);
	applicationContext.dispatch(hex_ioc_assembler_ApplicationAssemblerMessage.OBJECTS_BUILT);
	var __domainName_myService = hex_domain_DomainUtil.getDomain("myService",hex_domain_Domain);
...
hex_event_CompositeDispatcher.prototype = {
	...
	,addListener: function(listener) {
		throw new js__$Boot_HaxeError(new hex_error_UnsupportedOperationException("'addListener' is not supported in '" + hex_log_Stringifier.stringify(this) + "'",{ fileName : "CompositeDispatcher.hx", lineNumber : 76, className : "hex.event.CompositeDispatcher", methodName : "addListener"}));
	}
	,removeListener: function(listener) {
		throw new js__$Boot_HaxeError(new hex_error_UnsupportedOperationException("'removeListener' is not supported in '" + hex_log_Stringifier.stringify(this) + "'",{ fileName : "CompositeDispatcher.hx", lineNumber : 81, className : "hex.event.CompositeDispatcher", methodName : "removeListener"}));
	}
	,removeAllListeners: function() {
		if(!this._isSealed) {
			var _g = 0;
	...
	,isRegistered: function(listener,messageType) {
		throw new js__$Boot_HaxeError(new hex_error_UnsupportedOperationException("'isRegistered' is not supported in '" + hex_log_Stringifier.stringify(this) + "'",{ fileName : "CompositeDispatcher.hx", lineNumber : 113, className : "hex.event.CompositeDispatcher", methodName : "isRegistered"}));
	}
	,hasHandler: function(messageType,scope) {
		var b = false;
		var _g = 0;
...
hex_service_stateful_StatefulService.prototype = $extend(hex_service_AbstractService.prototype,{
	...
	,setConfiguration: function(configuration) {
		if(this._inUse) {
			this._throwExecutionIllegalStateError("setConfiguration");
		}
...
var mock_IMockService = function() { };
$hxClasses["mock.IMockService"] = mock_IMockService;
mock_IMockService.__name__ = ["mock","IMockService"];
var mock_MockMessage = function() { };
$hxClasses["mock.MockMessage"] = mock_MockMessage;
mock_MockMessage.__name__ = ["mock","MockMessage"];
var mock_MockModule = function(config) {
	hex_module_Module.call(this);
	if(config != null) {
		this._addStatefulConfigs([config]);
	}
};
$hxClasses["mock.MockModule"] = mock_MockModule;
mock_MockModule.__name__ = ["mock","MockModule"];
mock_MockModule.__super__ = hex_module_Module;
mock_MockModule.prototype = $extend(hex_module_Module.prototype,{
	onUpdate: function(now) {
		console.log(HxOverrides.dateStr(now));
	}
	,_getRuntimeDependencies: function() {
		return new hex_module_dependency_RuntimeDependencies();
	}
	,__class__: mock_MockModule
});
var mock_MockService = function() {
	hex_service_stateful_StatefulService.call(this);
	console.log("MockService");
	var yourTimer = new haxe_Timer(1000);
	yourTimer.run = $bind(this,this._notify);
};
$hxClasses["mock.MockService"] = mock_MockService;
mock_MockService.__name__ = ["mock","MockService"];
mock_MockService.__interfaces__ = [mock_IMockService];
mock_MockService.__super__ = hex_service_stateful_StatefulService;
mock_MockService.prototype = $extend(hex_service_stateful_StatefulService.prototype,{
	_notify: function() {
		this.getDispatcher().dispatch(mock_MockMessage.EVENT,[new Date()]);
	}
	,__class__: mock_MockService
});
...
hex_service_stateful_StatefulService.__INJECTION_DATA = { c : { a : []}, p : [], m : [], pc : [{ m : "createConfiguration", a : [], o : 0}], pd : []};
...
hex_view_viewhelper_ViewHelperManagerMessage.VIEW_HELPER_CREATION = "onViewHelperCreation";
hex_view_viewhelper_ViewHelperManagerMessage.VIEW_HELPER_RELEASE = "onViewHelperRelease";
...
mock_MockMessage.EVENT = "TestMessage.event";
mock_MockService.__INJECTION_DATA = { c : { a : []}, p : [], m : [], pc : [{ m : "createConfiguration", a : [], o : 0}], pd : []};
Main.main();
